...
typedef struct ms_ecall_process_stats_t {
	...
	const char* ms_column_name;
	size_t ms_column_name_len;
	int ms_op_code;
	char* ms_out_mode_buf;
	uint32_t ms_out_mode_buf_len;
	double* ms_result;
...
static sgx_status_t SGX_CDECL sgx_ecall_process_stats(void* pms)
{
	...
	const char* _tmp_column_name = __in_ms.ms_column_name;
	size_t _len_column_name = __in_ms.ms_column_name_len ;
	char* _in_column_name = NULL;
	char* _tmp_out_mode_buf = __in_ms.ms_out_mode_buf;
	uint32_t _tmp_out_mode_buf_len = __in_ms.ms_out_mode_buf_len;
	size_t _len_out_mode_buf = _tmp_out_mode_buf_len;
	char* _in_out_mode_buf = NULL;
	...
	CHECK_UNIQUE_POINTER(_tmp_column_name, _len_column_name);
	CHECK_UNIQUE_POINTER(_tmp_out_mode_buf, _len_out_mode_buf);
	...
	if (_tmp_column_name != NULL && _len_column_name != 0) {
		_in_column_name = (char*)malloc(_len_column_name);
		if (_in_column_name == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_column_name, _len_column_name, _tmp_column_name, _len_column_name)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

		_in_column_name[_len_column_name - 1] = '\0';
		if (_len_column_name != strlen(_in_column_name) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_tmp_out_mode_buf != NULL && _len_out_mode_buf != 0) {
		if ( _len_out_mode_buf % sizeof(*_tmp_out_mode_buf) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_out_mode_buf = (char*)malloc(_len_out_mode_buf)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_out_mode_buf, 0, _len_out_mode_buf);
	}
	...
	_in_retval = ecall_process_stats((const char*)_in_signed_data, _tmp_signed_data_len, (const uint8_t*)_in_sig1, _tmp_sig1_len, (const uint8_t*)_in_sig2, _tmp_sig2_len, (const uint8_t*)_in_ciphertext, _tmp_ciphertext_len, (const uint8_t*)_in_iv, _tmp_iv_len, (const uint8_t*)_in_mac, (const char*)_in_column_name, __in_ms.ms_op_code, _in_out_mode_buf, _tmp_out_mode_buf_len, _in_result);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_out_mode_buf) {
		if (memcpy_verw_s(_tmp_out_mode_buf, _len_out_mode_buf, _in_out_mode_buf, _len_out_mode_buf)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
